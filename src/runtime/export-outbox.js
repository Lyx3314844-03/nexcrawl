...
import { deliverExportPlan } from './export-manager.js';
...
function sortEntries(items = []) {
  return [...items].sort((left, right) =>
    ...
    || String(left.createdAt ?? '').localeCompare(String(left.createdAt ?? '')));
}
...
export class ExportOutbox {
  constructor({ projectRoot = process.cwd(), path } = {}) {
    ...
    this.path = path ?? join(this.storageDir, 'export-outbox.json');
    this.entries = [];
  ...
  async enqueueFailedDelivery({ workflowName, jobId, output, serialized, format, destination, manifest } = {}) {
    if (!destination || !serialized) {
      return null;
    }
    ...
    const entry = {
      ...
      attemptCount: 0,
      status: 'queued',
      lastError: manifest?.reason ?? null,
      ...
      output,
      serialized,
      lastStatus: manifest?.status ?? null,
    };
  ...
  async processDue({ logger } = {}) {
    ...
    for (const entry of due) {
      ...
      const response = await deliverExportPlan({
      ...
      entry.lastStatus = response.status ?? null;
      if (response.delivered) {
        ...
        const backoffMs = Math.max(1000, Number(entry.output?.retryBackoffMs ?? 1000));
...
export class ExportOutboxService {
  constructor({ projectRoot = process.cwd(), logger, pollIntervalMs = 10000 } = {}) {
    this.projectRoot = projectRoot;
    this.logger = logger ?? createLogger({ component: 'export-outbox' });
    this.pollIntervalMs = pollIntervalMs;
    this.outbox = new ExportOutbox({ projectRoot });
    this.timer = null;
  }
  ...
  async enqueueFailedDelivery(input) {
    return this.outbox.enqueueFailedDelivery(input);
  }
